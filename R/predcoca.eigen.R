...
predcoca.eigen <- function(y, x, R0 = NULL, n.axes = NULL, nam.dat = NULL) {
  ...
  if (p < q) {
    for (s in seq_len(nq1)) {
      PI1 <- Conj(t(C1)) %*%
        solve(C1 %*% invYc %*% Conj(t(C1))) %*%
        C1 %*%
        invYc
      PI2 <- Conj(t(C2)) %*%
        solve(C2 %*% invXc %*% Conj(t(C2))) %*%
        C2 %*%
        invXc
      ZKZt <- (diag(1, nrow = nrow(PI1), ncol = ncol(PI1)) - PI1) %*%
        Z %*%
        invXc %*%
        (diag(1, nrow = nrow(PI2), ncol = ncol(PI2)) - PI2) %*%
        Conj(t(Z))
    ...
    for (s in seq_len(nq1)) {
      ...
      PI2 <- Conj(t(C2)) %*%
        ...
        C2 %*%
        invXc
      ZtKZ <- (diag(1, nrow = nrow(PI2), ncol = ncol(PI2)) - PI2) %*%
        Conj(t(Z)) %*%
        invYc %*%
        (diag(1, nrow = nrow(PI1), ncol = ncol(PI1)) - PI1) %*%
        Z
  ...
  retval <- list(
    ...
    R0 = R0,
    Ychi = list(Ychi1 = Ychi1, Ychi2 = Ychi2),
    call = match.call(),
    loadings = NULL,
    fitted = NULL,
    varianceExp = NULL,
