...
#' @name cocorresp-internal
predcoca.simpls <- function(y, x, R0 = NULL, n.axes = NULL, nam.dat = NULL) {
  if (is.null(nam.dat)) {
  ...
  retval <- list(
    ...
    varianceExp = list(Xblock = pls.res$Xvar, Yblock = pls.res$Yvar),
    totalVar = list(Xblock = pls.res$Xtotvar, Yblock = pls.res$Ytotvar),
    lambda = NULL,
    n.axes = n.axes,
