...
#' @export
print.summary.predcoca <- function(
  x,
  ...
  if (!is.null(lambda)) {
    ...
    cat("\nY-block: variance explained in", x$namY, "(response) \n", sep = " ")
    print(format(Yvar.mat, digits = digits), ..., print.gap = 2, quote = FALSE)
    cat("\nX-block: variance explained in", x$namX, "(predictor) \n", sep = " ")
    print(format(Xvar.mat, digits = digits), ..., print.gap = 2, quote = FALSE)
  }
  invisible(x)
}
