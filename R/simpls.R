...
simpls <-
  function(X, Y, ncomp, stripped = FALSE, ...) {
    validate_count(ncomp, "ncomp")
    if (!is.matrix(X) || !is.numeric(X) || !all(is.finite(X))) {
      stop("X must be a finite numeric matrix")
    }
    Y <- as.matrix(Y)
    if (!is.numeric(Y) || !all(is.finite(Y)) || nrow(X) != nrow(Y)) {
      stop("Y must be finite numeric data with the same rows as X")
    }
    if (ncomp > min(nrow(X), ncol(X), ncol(Y))) {
      stop("ncomp exceeds matrix dimensions")
    }
