...
Swapping communities preserves symmetric weighting only when `symmetric = TRUE` or identical row weights are supplied; the default weights come from the response community. The messages printing during fitting are for information; some species contained no data and hence were removed prior to fitting. This data processing step could have been performed ahead of time via
...
Some additional control is afforded by the `plot()` method, but good plots of the fitted Co-CA will often require the use of lower-level functions such as the `points()` and `vegan::scores()` methods, as well as using appropriate sets of scores or loadings.
